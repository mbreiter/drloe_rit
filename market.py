...
class RITEnvironment(gym.Env):
    ...
    def __init__(self, params):
        ...
        self.liquidate = params['liquidate']
        self.direction = np.sign(self.liquidate)
        ...
        self.status, self.time, self.end_time = self._get_tick_status()
        self.start_time = params['start_time'] if params['start_time'] is not None else self.time
        self.end_time = params['end_time'] if params['end_time'] is not None else self.end_time - 10
    ...
    def reset(self):
        ...
        while self.status != 'ACTIVE' and self.time >= self.start_time:
            self.status, self.time, _ = self._get_tick_status()
    ...
    def _get_state(self):
        ...
        state_info = dict(zip(OH_COLS, [self.time,
                                        self.end_time,
                                        self.liquidate,
                                        self.position,
                                        self.pending,
    ...
    def step(self, action):
        ...
        self.position, self.cost = self._get_position()
        self.pending = self._get_pending()

        # get the state
        obs = self._get_state()

        # check if we are done
        if self.time >= self.end_time or abs(self.position) == abs(self.liquidate):
            done, info = True, 'trading is done.\t'
            info += 'timeout: {}\tinventory met: {}'.format(self.time == self.end_time,
                                                            abs(self.position) == abs(self.liquidate))
            reward += self._calc_reward(self.liquidate, self.cost, portion=1, done=True)
        else:
    ...
    def _delayed_helper(self, order):
        ...
        if filled != order['quantity_filled']:
            executed_quantity = abs(filled - order['quantity_filled'])
            ...
            self.order_history.loc[self.order_history['order_id'] == oid, 'quantity_filled'] = filled
    ...
    def _calc_reward(self, quantity, vwap_e, portion=1, done=False):
        ...
        if self.reward_signal['collection'] == 'terminal':
            reward = abs(quantity) * (vwap_e - vwap_m) * self.direction if done else 0
        else:
            if self.reward_signal['method'] == 'VWAP_PNL':
                reward = abs(quantity) * vwap_m * -self.direction if done else abs(quantity) * vwap_e * self.direction
    ...
    def _get_position(self):
        ...
        if resp.ok:
            ...
            current_position, cost = securities[0]['position'], securities[0]['vwap']
